    ...
    public abstract class FormattingPluginBase : PluginBase, IFormattingPlugin {
        ...
        protected void AddDefaultSlackFields(PersistentEvent ev, List<SlackMessage.SlackAttachmentFields> attachmentFields, bool includeUrl = true) {
            ...
            if (ev.Type == Event.KnownTypes.Error || ev.Type == Event.KnownTypes.NotFound)
                actions.Add($"• <{AppConfiguration.Current.BaseURL}/stack/{ev.StackId}/mark-fixed|Mark event as fixed>");
            actions.Add($"• <{AppConfiguration.Current.BaseURL}/stack/{ev.StackId}/stop-notifications|Stop sending notifications for this event>");
            actions.Add($"• <{AppConfiguration.Current.BaseURL}/project/{ev.ProjectId}/manage?tab=integrations|Change your notification settings for this project>");
        ...
        protected string GetSlackEventUrl(string eventId, string message = null) {
            var parts = new List<string> { $"{AppConfiguration.Current.BaseURL}/event/{eventId}" };
